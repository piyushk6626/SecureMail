...
This repository has completed **Steps 0–6** of `build_plan.md`: package layout,
...
per-certificate extraction, and offline chain validation with RFC 9525 identity
matching. The live CLI command is `securemail analyze`. The next step to
implement is **Step 7** (versioned rule packs and forward secrecy).
